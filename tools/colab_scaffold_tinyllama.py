# CONSULT-2 follow-up (Colab GPU): TinyLlama scaffold rescue: hard-only (replication) vs 30hard+6easy. Predicts fmt holds in scaffold (vs 12->2 collapse); acc >= hard-only.
# Paste this whole file into ONE Colab cell and run. Paste ALL cell output back.
# TinyLlama scaffold rescue: hard-only (replication) vs 30hard+6easy. Predicts fmt holds in scaffold (vs 12->2 collapse); acc >= hard-only.
...
ARMS = [('hard-only', 8, 16, 'hard36'), ('scaffold', 8, 16, 'scaffold')]
